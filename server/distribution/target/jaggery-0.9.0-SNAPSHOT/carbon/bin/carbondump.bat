...
REM find $CARBON_HOME -iname "*" -type f | xargs md5sum > $OUTPUT_DIR/checksum_values.txt
...
cd %OUTPUT_ROOT_DIR%
if exist %CARBON_DUMP_HOME%\carbondump-%DATE_TIME%.zip del %CARBON_DUMP_HOME%\carbondump-%DATE_TIME%.zip
jar cvf %CARBON_DUMP_HOME%\carbondump-%DATE_TIME%.jar carbondump-%DATE_TIME%
cd %CARBON_DUMP_HOME%
rename carbondump-%DATE_TIME%.jar carbondump-%DATE_TIME%.zip

echo carbondump: %CARBON_DUMP_HOME%\carbondump-%DATE_TIME%.zip
